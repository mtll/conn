  ...
  (cl-flet ((read-ev (&optional seconds)
    ...
    (if seconds
      ...
      (conn-read-args (nil :prompt prompt
                           ...
                           :overriding-map conn-dispatch-read-char-map
                           :pre (lambda (cmd)
                                  (run-hook-with-args
...
(conn-define-argument-command ((arg conn-dispatch-prefix-arg)
...
(conn-define-argument-command ((arg conn-read-char-input-method)
...
(cl-defmethod conn-target-finder-select ((_state conn-dispatch-retargetable-mixin))
  (conn-with-dispatch-event-handlers
    (:handler
     ...
     ( :display ()
       (list
        (when (not conn--dispatch-always-retarget)
          "\\[retarget] retarget")
