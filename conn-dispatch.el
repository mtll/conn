...
(eval-and-compile
  (defun conn--expand-dispatch-handler (tag body)
    `(push (oclosure-lambda (conn-dispatch-handler
                             ...
                             (handler-display
                              ,(if-let* ((v (alist-get :display body)))
                                   `(lambda ,@v)
                                 '#'ignore))
                             (handler-predicate
                              ,(if-let* ((v (alist-get :predicate body)))
                                 ...
                                 '#'ignore))
                             (handler-annotation
                              ,(if-let* ((v (alist-get :annotation body)))
                                   `(lambda ,@v)
                                 '#'ignore))
                             (handler-documentation
                              ,(if-let* ((v (alist-get :documentation body)))
                                   `(lambda ,@v)
                                 '#'ignore))
                             (handler-keymap
                              ,(car (alist-get :keymap body))))
               ,@(or (alist-get :update body)
                     (list '(&rest _) nil)))
              ...
              (conn-reference-page
                (:eval
                 (substitute-command-keys
                  "\\<conn-dispatch-read-char-map>Perform the next dispatch in a loop.
...
(defconst conn-dispatch-select-mode-map-alist
  `((conn-dispatch-select-mode . ,conn-dispatch-read-char-map)))
...
(define-minor-mode conn-dispatch-select-mode
  ...
  (if conn-dispatch-select-mode
      (progn
        (cl-pushnew 'conn-dispatch-select-mode-map-alist
...
(cl-defmethod conn-argument-display ((_handler conn-dispatch-select-handler))
  (list
   (propertize
