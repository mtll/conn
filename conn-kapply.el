...
(defun conn--kapply-describe-macro (&optional macro)
  (if macro
      (list (conn--kmacro-display (kmacro--keys macro) 20)
            (kmacro--counter macro)
            (kmacro--format macro))
    (list (conn--kmacro-display last-kbd-macro 20)
          kmacro-counter
          kmacro-counter-format)))

(defun conn-push-kapply-command-history (command &rest args)
  (pcase-let (((or `(,cmd ,fn) fn) command))
    (conn-push-command-history
     (oclosure-lambda (conn-previous-command
                       (command cmd)
                       (function fn)
                       (arguments `(,@(butlast args)
                                    ,(conn--kapply-describe-macro))))
         ()
       (apply fn args)
       (setf (car (last arguments))
             (conn--kapply-describe-macro))))))
...
(defun conn--kapply-multi-file-matches (files
                                        string
                                        regexp-flag
                                        delimited-flag
                                        &optional
        ...
        ((collect-matches (buffer)
         ...
         (next ()
           (while-let ((next (and (null matches)
                                  (pop files))))
             (cond* ((bind-and* (buffer (get-file-buffer next)))
                     (collect-matches buffer))
                    ((check-file next)
...
(cl-defmethod conn-kapply-on-matches ((thing (eql multi-file))
  ...
  (conn->f pipeline
    (conn--kapply-multi-file-matches
     ...
     regexp-flag
     delimited-flag))
  (conn-push-kapply-command-history 'conn-kapply-on-matches
                                    thing nil nil
                                    nil
                                    regexp-flag
                                    delimited-flag
       ...
       (pcase state
         (:record
          (let ((hl (make-overlay (point) (point))))
            (overlay-put hl 'priority 2000)
            (overlay-put hl 'face 'query-replace)
            (if (not conn-kapply-query-on-record)
...
(defun conn-kapply-save-excursion (iterator &optional always)
   ...
   (let (saved-excursions)
     (lambda (iterator state)
       (pcase state
         (:cleanup
          (funcall iterator state)
          (when (or conn-kmacro-apply-error always)
...
(cl-defmethod conn-argument-update :around ((arg conn-kapply-macro-argument)
                                            cmd
                                            break)
  (cl-call-next-method
   arg cmd
   (lambda ()
     (setf (conn-kapply-macro-argument-count arg)
...
(cl-defmethod conn-argument-value ((arg conn-kapply-macro-argument))
  (let ((register (conn-kapply-macro-argument-register arg)))
    (list (pcase (conn-argument--value arg)
            ('record
             (let ((macro (when register (get-register register))))
               (lambda (it &optional count)
                 (conn-kmacro-apply it count macro))))
            ('apply
            ...
            ((and cmd (or 'append 'append-skip-exec))
             ...
             (lambda (it &optional count)
               (conn-kmacro-apply-append it count
                                         (eq cmd 'append-skip-exec))
               (when register
            ...
            ('step-edit
             (when register
               (kmacro-split-ring-element (get-register register)))
             (lambda (it &optional count)
               (conn-kmacro-apply-step-edit it count)
               (when register
                 (set-register register (kmacro-ring-head))))))
          (conn-kapply-macro-argument-count arg))))
       ...
       (pipeline
        (conn-composite-argument
         (nconc (or pipeline
                ...
                (ensure-list extra))))
       ((or `(,app ,_count) app)
        (or applier (conn-kapply-macro-argument))))
...
(cl-defmethod conn-change-thing-do ((_thing (eql kapply))
                                    _arg
                                    transform
                                    &optional
                                    _check-bounds
  ...
  (conn-read-args (conn-kapply-matches-state
                   :prompt "Kapply in Thing")
      ((`(,thing ,arg ,subregions-p)
        (conn-kapply-matches-thing-argument))
       (transform (conn-transform-argument transform))
       (regexp-flag
        (conn-boolean-argument "regexp"
                               'regexp
  ...
  (conn-read-args (conn-kapply-matches-state
    ...
    (save-excursion
      ...
      (when pipeline
        (conn-push-command-history
         (oclosure-lambda (conn-previous-command
                           (command 'conn-kapply-on-word)
                           (arguments
                            (list thing arg transform
                                  (conn--kapply-describe-macro))))
             ()
           (apply #'conn-kapply-on-matches
                  `(,@arguments
                    nil nil t
                    ,(thing-at-point 'word)
                    ,pipeline))
           (setf (car (last arguments)) (conn--kapply-describe-macro))))))
    pipeline))
  ...
  (conn-read-args (conn-read-thing-state
    ...
    (pcase (conn-bounds-of thing arg)
      ((conn-bounds `(,beg . ,end))
       ...
       (when pipeline
         (conn-push-kapply-command-history '(conn-kapply-on-highlights-in-thing
  ...
  (let ((rmm (bound-and-true-p rectangle-mark-mode))
    ...
    (pcase (conn-bounds-of thing arg)
      ((and bounds (conn-bounds-get :subregions transform))
       ...
       (conn-push-kapply-command-history '(conn-kapply-on-things
                                           conn-kapply-on-things-do)
                                         thing
                                         arg
                                         transform
...
(defun conn-kapply-count-iterator ()
  (interactive)
  (conn-read-args (conn-kapply-state
      ...
      ((pipeline (conn-composite-argument
                  (list (conn-kapply-query-argument)
                        ...
                        (conn-kapply-undo-argument))))
       (`(,applier ,count) (conn-kapply-macro-argument)))
    (unless (= count 1)
      (conn-kapply-macro
       (lambda (it) (funcall applier it (or (and count (1- count)) 0)))
    ...
    (let ((macro (kmacro-ring-head)))
      (conn-push-command-history
       (oclosure-lambda (conn-previous-command
                         (command 'conn-kapply-count)
                         (arguments (list `(or current-prefix-arg ,count)
                                          (conn--kapply-describe-macro))))
           ()
         (conn-kapply-macro
          (lambda (it)
            (conn-kmacro-apply it
                               (or current-prefix-arg count)
                               macro))
          (conn-kapply-infinite-iterator)
          `(conn-kapply-relocate-to-region
            conn-kapply-open-invisible
               ...
               (next ()
                 (when multi-isearch-next-buffer-current-function
                   (condition-case _
                     ...
                     (error nil)))))
      (cl-macrolet
          ((with-protected-isearch-vars (body)
               ...
               ((rst restrict)
                (pipeline (conn-composite-argument
                           (list (conn-kapply-state-argument)
                                 ...
                                 (conn-kapply-window-conf-argument)
                                 (conn-kapply-undo-argument))))
                (`(,applier ,_count) (conn-kapply-macro-argument)))
             (setf restrict rst)
             (collect-matches)
             (conn-kapply-macro applier
                                iterator
                                `(conn-kapply-relocate-to-region
                                  conn-kapply-open-invisible
                                  conn-kapply-pulse-region
...
(defun conn-kapply-add-points-at-things-in-region ()
  ...
  (unless (eq (current-buffer) conn--kapply-at-points-buffer)
    (user-error "Kapply on points not active in buffer"))
  (pcase (conn-bounds-of 'conn-things-in-region nil)
    ((conn-bounds-get :subregions)
     (conn-> subregions
             (mapcar (lambda (b) (car (conn-bounds b))))
             (apply #'conn--kapply-point-cursor-at))
...
(defun conn-kapply-add-points-at-symbols-in-thing ()
  (interactive)
  (unless (eq (current-buffer) conn--kapply-at-points-buffer)
    (user-error "Kapply on points not active in buffer"))
  (when-let* ((string (thing-at-point 'symbol)))
    (conn-read-args (conn-kapply-matches-state
                     :prompt "Kapply on Symbol Within Thing")
        ((`(,thing ,arg) (conn-thing-argument))
         (transform (conn-transform-argument)))
...
(defun conn-kapply-at-points-undo ()
  ...
  (unless (eq (current-buffer) conn--kapply-at-points-buffer)
    (user-error "Kapply on points not active in buffer"))
  (goto-char (caar conn--kapply-at-points))
  (set-marker (caar conn--kapply-at-points) nil)
  (mapc #'delete-overlay (cdr (pop conn--kapply-at-points))))

(defun conn-kapply-at-points-emacs ()
  (interactive)
  (unless (eq (current-buffer) conn--kapply-at-points-buffer)
    (user-error "Kapply on points not active in buffer"))
  (conn-kapply-at-points-begin
   #'conn-kmacro-apply
   (list (lambda (it)
...
(defun conn-kapply-at-points-apply ()
  ...
  (conn-read-args (conn-kapply-state
      ...
      ((pipeline
        (conn-composite-argument
         (list (conn-kapply-query-argument nil)
               (conn-kapply-window-conf-argument nil)
               (conn-kapply-restrictions-argument nil)
               (conn-kapply-undo-argument))))
         ...
         (conn-kapply-region-iterator
          (conn-protected-let* ((buffer-read-only t)
                                ...
                                ( conn--kapply-at-points nil
                                  (mapc (lambda (list)
                                          (set-marker (car list) nil)
                                          (mapc #'delete-overlay (cdr list)))
                                        conn--kapply-at-points)))
            (pcase-setq `(,applier . ,pipeline)
                        (save-current-buffer
            ...
            (let ((pts nil)
                  (testfn (lambda (p1 p2)
                            (= (overlay-start p1)
                               (overlay-start p2)))))
              (dolist (pt (apply #'append (mapcar #'cdr conn--kapply-at-points)))
...
(defun conn-kapply-at-points-no-cursor ()
  (interactive)
  (conn--kapply-at-points
   (lambda ()
     (conn-with-recursive-stack 'conn-kapply-at-points-state
...
(defun conn-kapply-at-points-in-region ()
  (interactive)
  (conn--kapply-at-points
   (lambda ()
     (conn-with-recursive-stack 'conn-kapply-at-points-state
