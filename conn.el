...
(defun conn--dispatch-ensure-region-buffer (region)
...
(defun conn--dispatch-ensure-overlay-buffer (dot)
    ...
    (pcase state
      (:finalize
       (pcase-dolist (`(,beg . ,end) regions)
         ...
         (set-marker end nil)))
      (_
       (conn--dispatch-ensure-region-buffer (pop regions))))))
    ...
    (pcase state
      ...
      (_
       (when-let ((pt (pop points)))
         (conn--dispatch-ensure-region-buffer (cons pt pt)))))))
...
(defun conn--dot-iterator (dots &optional reverse)
  ...
  (lambda (state)
    (pcase state
      ...
      (_
       (conn--dispatch-ensure-overlay-buffer (pop dots))))))

(defun conn--dispatch-merge-undo (iterator)
...
(defun conn--dispatch-save-excursion (iterator)
  (let (dispatch-saved-excursions)
    (lambda (state)
      (pcase state
        (:finalize
         (funcall iterator state)
         (pcase-dolist (`(,buffer ,pt . ,saved) dispatch-saved-excursions)
...
(defun conn--dispatch-save-restriction (iterator)
  (let (dispatch-saved-restrictions)
    (lambda (state)
      (pcase state
        (:finalize
         (funcall iterator state)
         (pcase-dolist (`(,buffer ,beg . ,end) dispatch-saved-restrictions)
           (with-current-buffer buffer
             (widen)
             (narrow-to-region beg end))))
        (_
...
(defun conn--delete-label-overlay (overlay)
  (delete-overlay (overlay-get overlay 'prefix-overlay))
...
(defun conn--create-label-overlays (labels places prefix-length)
  (let (overlays)
    (condition-case _
        (pcase-dolist (`(,window . ,points) places)
          (with-current-buffer (window-buffer window)
            (dolist (pt points)
              (let* ((pt (+ prefix-length pt))
                     (label (pop labels))
                     ...
                     (prefix-ov (make-overlay (- pt prefix-length) pt)))
                (when (null label)
                ...
                (if (null overlap)
                  ...
                  (mapc (lambda (o)
                        ...
                        overlap))
                (push ov overlays)
                (overlay-put prefix-ov 'priority 3000)
                (overlay-put prefix-ov 'conn-overlay t)
                (overlay-put prefix-ov 'category 'conn-dispatch-prefix-overlay)
                (overlay-put prefix-ov 'window window)
                (overlay-put prefix-ov 'face 'conn-read-string-match-face)
                (overlay-put ov 'prefix-overlay prefix-ov)
  ...
  (let* ((string (conn--read-string-with-timeout nil t))
    ...
    (unwind-protect
        (cl-loop
         ...
         (setq overlays (conn--create-label-overlays labels places (length string)))
         (while (cdr overlays)
