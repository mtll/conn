...
;; Author: David Feller
;; Version: 0.1
;; Package-Requires: ((emacs "29.4") (compat "31.0.0.0") posframe conn)
;;
      ...
      (unless window-side
        (dolist (buffer (buffer-list frame))
          (when (and (buffer-live-p buffer)
            ...
            (if (switch-to-prev-buffer-skip-p skip window buffer)
                (setf skipped (or skipped buffer))
              (setf new-buffer buffer)
              (cl-pushnew new-buffer found-buffers)
              (when (length= found-buffers conn-posframe-buffer-context-lines)
                (throw 'found t))))))
