...
(defun conn-consult-ripgrep-thing (thing arg transform)
  ...
  (pcase (conn-bounds-of thing arg)
    ((conn-bounds `(,beg . ,end) transform)
     (deactivate-mark)
     (setf this-command 'consult-ripgrep)
     (consult-ripgrep
      nil
...
(defun conn-consult-line-thing (thing arg transform)
  ...
  (pcase (conn-bounds-of thing arg)
    ((conn-bounds `(,beg . ,end) transform)
     (deactivate-mark)
     (setf this-command 'consult-line)
     (consult-line
      (when (and (< (- end beg) 1000)
                 (= (count-lines beg end) 1))
        (funcall conn-completion-region-quote-function
...
(defun conn-consult-line-multi-thing (thing arg transform)
  ...
  (pcase (conn-bounds-of thing arg)
    ((conn-bounds `(,beg . ,end) transform)
     (deactivate-mark)
     (setf this-command 'consult-line-multi)
     (consult-line-multi
      nil
      (when (and (< (- end beg) 1000)
                 (= (count-lines beg end) 1))
...
(defun conn-consult-git-grep-thing (thing arg transform)
  ...
  (pcase (conn-bounds-of thing arg)
    ((conn-bounds `(,beg . ,end) transform)
     (deactivate-mark)
     (setf this-command 'consult-git-grep)
     (consult-git-grep
      nil
      (when (and (< (- end beg) 1000)
                 (= (count-lines beg end) 1))
...
(defun conn-consult-find-thing (thing arg transform)
  ...
  (pcase (conn-bounds-of thing arg)
    ((conn-bounds `(,beg . ,end) transform)
     (deactivate-mark)
     (setf this-command 'consult-find)
     (consult-find
      nil
      (when (and (< (- end beg) 1000)
                 (= (count-lines beg end) 1))
  ...
  (let ((files nil)
    ...
    (cl-flet ((next ()
      ...
      (lambda (state)
        (pcase state
          (:cleanup
           (mapc #'delete-overlay curr-file))
          (:exit-current
           (mapc #'delete-overlay (cl-shiftf curr-file nil)))
          ((or :next :record)
